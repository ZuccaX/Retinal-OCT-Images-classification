...
from src.deeplearning.collect_results import collect_results as collect_all_results
...
def command_train_dl(args: argparse.Namespace) -> None:
    ...
    manifest_df = pd.read_csv(manifest_path)
    if args.eval_split == "test_final" and not args.final_test:
        raise ValueError("Use --final-test when running eval on test_final.")
...
def command_collect_results(args: argparse.Namespace) -> None:
    """Build one unified traditional+DL result table and protocol checks."""
    paths = make_paths()
    ensure_dirs([paths.tables_root, paths.logs_root])
    logger = setup_logger(paths.logs_root / "collect_results.log")
    output_path, markdown_path, issue_path, protocol_note_path, all_results = collect_all_results(
        paths.tables_root
    )
    logger.info("Saved all-model CSV: %s", output_path)
    logger.info("Saved all-model markdown: %s", markdown_path)
    logger.info("Saved protocol validation CSV: %s", issue_path)
    logger.info("Saved protocol note: %s", protocol_note_path)
    logger.info("Total merged result rows: %d", len(all_results))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_dl = subparsers.add_parser("train-dl", help="Run DL framework (manifest-driven)")
    train_dl.add_argument(
        "--model",
        choices=["resnet18", "resnet34", "resnet50", "vgg16", "mobilenet_v2"],
        default="resnet18",
    )
    train_dl.add_argument("--eval-split", choices=["val_final", "val_raw_holdout", "test_final"], default="val_final")
    train_dl.add_argument("--final-test", action="store_true", help="Explicitly allow evaluation on test_final.")
    ...
    train_dl.add_argument("--data-mode", choices=["raw", "processed"], default="raw")
    train_dl.add_argument("--processed-profile", type=str, default="oct224_gray_png_v2")
    train_dl.add_argument("--freeze-backbone", action="store_true")
    train_dl.add_argument("--no-pretrained", action="store_true")
    ...
    collect = subparsers.add_parser(
        "collect-results",
        help="Collect traditional + DL results into one validated summary table",
    )
    collect.set_defaults(func=command_collect_results)
